...
class BaseExpensiTextInput extends Component {
    ...
    onBlur(event) {
        ...
        this.props.validate(this.props.name, [{pattern: /d{4,}/, message: 'Field should have more than 4 chars'}, {pattern: /d{,8}/, message: 'Field should have less than 8 chars'}])
    ...
    render() {
        ...
        return (
            <View>
                ...
                {!_.isEmpty(errorText) && (
                    <InlineErrorText>
                        {_.each(errorText, error => (
                             <Text style={[styles.mb1]}>{error}</Text>
                        ))}
                    </InlineErrorText>
                )}
            </View>
